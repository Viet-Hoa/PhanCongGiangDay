...
var DinhChiVuAn = (function () {
    var hoSoVuAnId = $("#HoSoVuAnID").val(),
        roleGiaiDoan = $("#roleGiaiDoan").val(),
        roleCongDoan = $("#roleCongDoan").val(),
        quyetDinhGroup = 4; // 4: đình chỉ vụ án

    var modalId = "#modalDinhChiVuAn",
        formEdit = "#formEditDinhChiVuAn",
        contentTab = "#contentDinhChiVuAn",
        selectNgayTao = "#selectNgayTaoDinhChiVuAn";
    ...
    function updateForm() {
        $(document).on("submit", formEdit, function () {
            ...
            $().CKEditorSetValForTextarea('ghi-chu-dinh-chi-vu-an-textarea');
            var _this = this;
            showLoadingOverlay(modalId + " .modal-content");
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if ($wrapperResponse.find(formEdit).length === 0) {
                        ...
                        $.notify({ message: "Cập nhật thông tin đình chỉ vụ án thành công" }, { type: "success" });
                    }
                    else {
                        $(modalId + " .modal-content").html(response);
                    }
...
var EditDinhChiVuAn = (function () {
    ...
    function init() {
        ...
        CKEDITOR.replace('ghi-chu-dinh-chi-vu-an-textarea');

        $(".datepicker").datetimepicker({
            format: 'DD/MM/YYYY'
        });

        $('#formEditDinhChiVuAn #file_upload').change(function (e) {
                    ...
                    $.ajax({
                        ...
                        success: function (result) {
                            if (result.status == 'success') {
                                $('#formEditDinhChiVuAn #DinhKemFile').val(result.fileName);
                                $('#formEditDinhChiVuAn #file_upload').parent().next('.error').remove();
                            }
                            else if (result.status == 'fail') {
                                if ($('#formEditDinhChiVuAn #file_upload').parent().next('.error').length == 0) {
                                    $('#formEditDinhChiVuAn #file_upload').parent().after('<div class="error has-error">' + result.msg + '</div>');
                                }
                            }
                        }
                    });
