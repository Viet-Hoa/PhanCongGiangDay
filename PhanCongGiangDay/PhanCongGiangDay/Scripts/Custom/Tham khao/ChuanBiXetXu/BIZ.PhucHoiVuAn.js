...
var PhucHoiVuAn = (function () {
    var hoSoVuAnId = $("#HoSoVuAnID").val(),
        roleGiaiDoan = $("#roleGiaiDoan").val(),
        roleCongDoan = $("#roleCongDoan").val(),
        quyetDinhGroup = 5; // 5: phục hồi vụ án

    var modalId = "#modalPhucHoiVuAn",
        formEdit = "#formEditPhucHoiVuAn",
        contentTab = "#contentPhucHoiVuAn",
        selectNgayTao = "#selectNgayTaoPhucHoiVuAn";
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.role == -1 || roleCongDoan == -1 || roleGiaiDoan == -1) {
                    $("#btnThemQuyetDinhPhucHoiHinhSu").addClass("add-disabled");
                    $("#btnSuaQuyetDinhPhucHoiHinhSu").addClass("edit-disabled");
                }
            }
        });
    }
    ...
    function updateForm() {
        $(document).on("submit", formEdit, function () {
            ...
            $().CKEditorSetValForTextarea('ghi-chu-phuc-hoi-vu-an-textarea');
            $().CKEditorSetValForTextarea('ly-do-phuc-hoi-vu-an-textarea');
            var _this = this;
            showLoadingOverlay(modalId + " .modal-content");
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if ($wrapperResponse.find(formEdit).length === 0) {
                        ...
                        $.notify({ message: "Cập nhật thông tin phục hồi vụ án thành công" }, { type: "success" });
                    }
                    else {
                        $(modalId + " .modal-content").html(response);
                    }
...
var EditPhucHoiVuAn = (function () {
    ...
    function init() {
        ...
        CKEDITOR.replace('ghi-chu-phuc-hoi-vu-an-textarea');
        CKEDITOR.replace('ly-do-phuc-hoi-vu-an-textarea');

        $(".datepicker").datetimepicker({
            format: 'DD/MM/YYYY'
        });

        $('#formEditPhucHoiVuAn #file_upload').change(function (e) {
                    ...
                    $.ajax({
                        ...
                        success: function (result) {
                            if (result.status == 'success') {
                                $('#formEditPhucHoiVuAn #DinhKemFile').val(result.fileName);
                                $('#formEditPhucHoiVuAn #file_upload').parent().next('.error').remove();
                            }
                            else if (result.status == 'fail') {
                                if ($('#formEditPhucHoiVuAn #file_upload').parent().next('.error').length == 0) {
                                    $('#formEditPhucHoiVuAn #file_upload').parent().after('<div class="error has-error">' + result.msg + '</div>');
                                }
                            }
                        }
                    });
