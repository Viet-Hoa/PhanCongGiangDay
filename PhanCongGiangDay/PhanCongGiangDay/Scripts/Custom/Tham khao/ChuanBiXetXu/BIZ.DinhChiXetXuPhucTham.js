$(function () {
    DinhChiXetXuPhucTham.init();
});

var DinhChiXetXuPhucTham = (function () {
    var hoSoVuAnId = $("#HoSoVuAnID").val(),
        roleGiaiDoan = $("#roleGiaiDoan").val(),
        roleCongDoan = $("#roleCongDoan").val(),
        quyetDinhGroup = 7; // 7: đình chỉ xét xử phúc thẩm

    var modalId = "#modalDinhChiXetXuPhucTham",
        formEdit = "#formEditDinhChiXetXuPhucTham",
        contentTab = "#contentDinhChiXetXuPhucTham",
        selectNgayTao = "#selectNgayTaoDinhChiXetXuPhucTham";
        ...
        $.ajax({
            ...
            success: function (response) {
                if (response.role == -1 || roleCongDoan == -1 || roleGiaiDoan == -1) {
                    $("#btnThemQuyetDinDinhChihHinhSu").addClass("add-disabled");
                    $("#btnSuaQuyetDinDinhChihHinhSu").addClass("edit-disabled");
                }
            }
        });
    }
    ...
    function updateForm() {
        $(document).on("submit", formEdit, function () {
            ...
            $().CKEditorSetValForTextarea('ghi-chu-dinh-chi-textarea');
            var _this = this;
            showLoadingOverlay(modalId + " .modal-content");
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if ($wrapperResponse.find(formEdit).length === 0) {
                        ...
                        $.notify({ message: "Cập nhật thông tin đình chỉ việc xét xử phúc thẩm thành công" }, { type: "success" });
                    }
                    else {
                        $(modalId + " .modal-content").html(response);
                    }
...
var EditDinhChiXetXuPhucTham = (function () {
    ...
    function init() {
        ...
        CKEDITOR.replace('ghi-chu-dinh-chi-textarea');

        $(".datepicker").datetimepicker({
            format: 'DD/MM/YYYY'
        });

        $('#formEditDinhChiXetXuPhucTham #file_upload').change(function (e) {
                    ...
                    $.ajax({
                        ...
                        success: function (result) {
                            if (result.status == 'success') {
                                $('#formEditDinhChiXetXuPhucTham #DinhKemFile').val(result.fileName);
                                $('#formEditDinhChiXetXuPhucTham #file_upload').parent().next('.error').remove();
                            }
                            else if (result.status == 'fail') {
                                if ($('#formEditDinhChiXetXuPhucTham #file_upload').parent().next('.error').length == 0) {
                                    $('#formEditDinhChiXetXuPhucTham #file_upload').parent().after('<div class="error has-error">' + result.msg + '</div>');
                                }
                            }
                        }
                    });
